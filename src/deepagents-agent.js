...
/**
 * DeepAgents Browser Agent with Multi-Provider Support
 *
 * This implementation uses LangChain DeepAgents to provide:
 * - Planning capabilities (built-in write_todos tool)
 ...
 * - Simple StateBackend for ephemeral storage
 */

/**
 * WeakMap to store session data including agents, cleanup functions, and session IDs
...
async function createPlaywrightTools(browserContext) {
  ...
  const mcpClient = new Client(
    {
      name: 'deepagents-playwright-client',
      version: '1.0.0'
    },
    {
      capabilities: {}
...
/**
 * Run DeepAgents agent with a specific browser context and automatic session management
 ...
 */
export async function runDeepAgentsAgent(prompt, browserContext, options = {}) {
  ...
  if (verbose) {
    console.log(`🤖 Running DeepAgents (${provider}) with shared context: "${prompt}"\n`);
  }
  ...
  try {
    ...
    if (!sessionData) {
      ...
      // Create the DeepAgents agent with StateBackend
      const agent = createDeepAgent({
        ...
        You have access to built-in planning and file system tools:
        - write_todos: Break down complex tasks into steps
        - ls, read_file, write_file, edit_file: Manage context and intermediate results
      ...
      if (verbose) {
        ...
        console.log(`🗂️  Backend: StateBackend (ephemeral)\n`);
      }
    } else {
      // Reusing existing session and agent
      if (verbose) {
        console.log(`♻️  AGENT: Reusing DeepAgents instance with ${sessionData.tools.length} tools\n`);
      }
    }
    ...
    if (verbose) {
      console.log('📡 Processing messages from DeepAgents:\n');
    }
    ...
    if (verbose) {
      ...
      console.log(`├─ Framework: deepagents`);
      console.log(`└─ Session ID: ${sessionData.sessionId}\n`);
    }

    // Note: MCP connections and agent are NOT cleaned up here
    ...
    // or when explicitly reset via runDeepAgentsAgent.resetSession()
    ...
    console.error('❌ Error running DeepAgents:', error.message);
 ...
 */
runDeepAgentsAgent.resetSession = async function(browserContext) {
